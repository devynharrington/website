...
My starting point was the [VCF Upgrade Planner path for VCF 9.0 to 9.1 with Automation](https://vmware.github.io/vcf-upgrade-planner/VCF-9.0-to-VCF-9.1-(VCF-Automation-included).html). Thank you to [William Lam](https://williamlam.com/) for turning the product dependencies into an order that was easy to follow. Generate a path using the versions and optional products actually deployed, then download the binaries before the window. The first VCF instance needs the full fleet-level set; later instances need fewer components, as listed in [KB 425089](https://knowledge.broadcom.com/external/article/425089/required-component-binaries-for-vcf-91.html).
...
After Operations is upgraded, complete the workflow that installs the local VCF License Server with its reserved FQDN and IP, then add it in **VCF Operations → Manage → Licensing → Registration** and select connected or disconnected mode. The License Server installation is automated by the supported 9.1 workflow, but the initial registration and licensing choices still need to be completed. Do this before the vCenter and ESX phases that depend on valid licensing.
